...
class MatchCapture {
    ...
    //            capture_object, capture_array, capture_meta_array }
    captureBegin(options) {
        options = options || {};
        const skip = this.instructionSetup();
        const bypassAll = (skip == true) ? true : false;
        const child = new MatchCapture(this.text, this.index, { ...options, parent: this, name, bypassAll });
        ...
        if (!skip && options.zeroMoreWhitespace) {
            let ix = sr_core_ts_1.scan_charNeAll(child.text, child.index, ' \t\n');
            if (ix == -1)
                ix = this.text.length;
            child.index = ix;
        }
        ...
        if (this.matchFail == false) {
            ...
            if (this.captureName || this.doCapture == true) {
                ...
                this.parent.store_capture({ bx: this.start, lx, name: this.captureName,
                    obj: this.capture_object }, this.doCapture);
            }
        }
    ...
    eof(options) {
        ...
        if (skip == false) {
            ...
            if (options.zeroMoreWhitespace) {
                ix = sr_core_ts_1.scan_charNeAll(this.text, ix, ' \t\n');
                if (ix == -1)
                    ix = this.text.length;
            }
    ...
    identifier(options = {}) {
        const skip = this.instructionSetup();
        if (skip == false) {
            const regexp_pattern = '[A-Za-z_]' + sr_core_ts_1.rxp.zeroMoreWord;
    ...
    literal(options = {}) {
        const skip = this.instructionSetup();
        if (skip == false) {
            const regexp_pattern = sr_core_ts_1.rxp.beginCapture + sr_core_ts_1.rxp.oneMoreDigits + sr_core_ts_1.rxp.endCapture +
                sr_core_ts_1.rxp.or +
                sr_core_ts_1.rxp.beginCapture + sr_core_ts_1.rxp.singleQuoteQuoted + sr_core_ts_1.rxp.endCapture +
                sr_core_ts_1.rxp.or + sr_core_ts_1.rxp.doubleQuoteQuoted;
            const opt = { ...options, skipSetup: true };
            this.matchRegExp(regexp_pattern, opt);
        }
        return this;
    ...
    match_processMatchFalse(options) {
    ...
    match_processMatchTrue(options, bx, lx) {
    ...
    matchRegExp(regexp, options) {
        ...
        if (skip == false) {
            ...
            if (remLx <= 0)
                ...
                const { matchBx, matchOx, matchLx } = sr_core_ts_1.regex_exec(this.text, ix, regexp);
                if (matchOx == 0) {
    ...
    nameStartOneMore() {
        ...
        if (skip == false) {
            ...
            const rv = sr_core_ts_1.regex_exec(this.text, ix, re);
    ...
    or(options) {
        ...
        if (options && options.repeatable && !options.isRepeatRun) {
            if (!this.repeat)
                this.repeat = [];
            this.repeat.push({ method: this.or, parm1: options });
        }
        return this;
    }
    // --------------------------------- MatchCapture.peekNext ---------------------------
    // static method. check that peekNext text or array of text is a match.
    static peekNext(text, ix, match) {
        let peekTrue = false;
        const matchItem = match;
        // advance past whitespace.
        ix = sr_core_ts_1.scan_charNeAll(text, ix, ' \t\n');
        if (ix == -1)
            ix = text.length;
    ...
    // property of this state object.
    // Repeat this match and run repeat method process until match is not true.
    repeatMatchText(matchText, options) {
        const skip = this.instructionSetup();
        if (!skip) {
            while (1) {
    ...
    //   })
    runRoutine(userRoutine, options) {
        ...
        if (options && options.repeatable && !options.isRepeatRun) {
            if (!this.repeat)
                this.repeat = [];
            this.repeat.push({ method: this.runRoutine, parm1: userRoutine, parm2: options });
        }
        return this;
    }
    // ---------------------------- was_ruyRoutine -----------------------------------
    // options: { captureName, zeroMatchOk, skipSetup, capture_object, repeatable }
    was_ruyRoutine(userRoutine, options) {
        const skip = this.instructionSetup({ noIncSeqn: true });
        if (skip) {
            this.instructSeqn += 1;
        }
    ...
    store_capture(item, toArray, errmsg) {
        let vlu;
        if (item.text)
            vlu = item.text;
        else
    ...
    // store the match text in capture_object
    // vlu - either match text or an object composed by a match method.
    store_capture_object(captureName, item) {
    ...
    // store the match text in capture_object
    // vlu - either match text or an object composed by a match method.
    old_storx_capture(captureName, captureToArray, cap, bx, lx, errmsg, cap_meta_array) {
        // capture to array.
        ...
        // store capture value as property name in capture_object.
            ...
            if ((typeof cap) == 'string') {
                this.capture_object[captureName] = { bx, lx, text: cap };
            }
            else {
    ...
    zeroMoreWhitespace(options) {
        ...
        if (options && options.repeatable && !options.isRepeatRun) {
            if (!this.repeat)
                this.repeat = [];
            this.repeat.push({ method: this.zeroMoreWhitespace, parm1: options });
        }
        return this;
    }
}
exports.MatchCapture = MatchCapture;
;
